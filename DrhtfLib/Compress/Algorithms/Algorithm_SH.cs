...
namespace DrhLib.Compress.Algorithms
{
		...
		ICompressAlgorithm
	{
		...
		public Algorithm_SH(IComputeRle rle, int channelCount)
		{
			this.rle = rle;
		...
		public void UpdateTable(Span<Color32> values, int channel, int lineIndex, AlgorithmKind kind)
		{
			...
			UpdateTableUtility.UpdateTableSH(values, channel, lineIndex, kind, info, true, ref info.MinZeroCount, rle);
		}
	}
}
